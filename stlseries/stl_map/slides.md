...
例如已知指向 "key" 的迭代器，想要插入 "kea"，那么指定指向 "key" 的迭代器就会让 insert 能更容易定位到 "kea" 要插入的位置。个人觉得意义不大，不建议使用，影响代码可读性。
...
emplace 系列只有个 try_emplace 是有用的，他会就地构造值对象，避免移动造成开销，用法举例：
...
map<int, MyClass> m;
m.try_emplace(1);                 // MyClass()
m.try_emplace(1, 42);             // MyClass(int)
m.try_emplace(1, "hell", 3.14f);  // MyClass(const char *, float)
```
