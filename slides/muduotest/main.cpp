...
struct jdate_clock {
    ...
    static constexpr duration jdiff() noexcept {
    ...
    static constexpr time_point from_system(std::chrono::system_clock::time_point sys_time) noexcept {
        return time_point{duration_cast<duration>(sys_time.time_since_epoch()) + jdiff()};
    }

    static constexpr std::chrono::system_clock::time_point to_system(time_point jdate_time) noexcept {
...
inline int32_t toJulianDay(std::chrono::year_month_day const &date) {
    return jdate_clock::from_system(std::chrono::sys_days{date}).time_since_epoch().count();
}

// muduo method:
inline int getJulianDayNumber(int year, int month, int day) {
    static char require_32_bit_integer_at_least[sizeof(int) >= sizeof(int32_t) ? 1 : -1];
...
// test and benchmark:
[[gnu::noinline]] void compute0(std::chrono::year_month_day const *date, int *__restrict result) {
...
[[gnu::noinline]] void compute1(std::chrono::year_month_day const *date, int *__restrict result) {
    for (size_t i = 0; i < batch; ++i) {
...
int main() {
    ...
    std::vector<std::chrono::year_month_day> date(batch);
    std::mt19937 rng;
    std::generate(date.begin(), date.end(), [&rng]() {
        int year = std::uniform_int_distribution(1970, 2070)(rng);
        int month = std::uniform_int_distribution(1, 12)(rng);
        int day = std::uniform_int_distribution(1, month == 2 ? 28 : 30)(rng);
